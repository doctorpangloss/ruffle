...
mod tests {
    ...
    #[test]
    fn baseline_shift_merges_like_other_properties() {
        let with_shift = || TextFormat {
            baseline_shift: Some(3.0),
            ..Default::default()
        };
        assert_eq!(
            with_shift()
                .merge_matching_properties(with_shift())
                .baseline_shift,
            Some(3.0)
        );
        let other = TextFormat {
            baseline_shift: Some(9.0),
            ..Default::default()
        };
        assert_eq!(
            with_shift().merge_matching_properties(other).baseline_shift,
            None
        );
    }
}
